...
public class UserRoleController implements Serializable {
    ...
    @EJB
    private entities.UserRoleFacade ejbFacade;
    ...
    public UserRole getSelected() {
        if (current == null) {
            ...
            current.setUserRolePK(new entities.UserRolePK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String create() {
        try {
            current.getUserRolePK().setUsername(current.getUser().getUsername());
    ...
    public UserRole getUserRole(entities.UserRolePK id) {
    ...
    public static class UserRoleControllerConverter implements Converter {
        ...
        entities.UserRolePK getKey(String value) {
            entities.UserRolePK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new entities.UserRolePK();
            key.setUsername(values[0]);
            key.setRole(values[1]);
            return key;
        }

        String getStringKey(entities.UserRolePK value) {
